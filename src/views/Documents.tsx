...
export default function Documents({ userRole }: { userRole: Role }) {
  ...
  const docs = [
    { id: 1, name: 'Employment Contract - John Doe', type: 'PDF', size: '2.4 MB', date: '2023-01-15' },
    { id: 2, name: 'NDA - Jane Smith', type: 'PDF', size: '1.1 MB', date: '2023-03-01' },
  ];
    ...
    <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden flex flex-col h-[calc(100vh-8rem)]">
      <div className="px-6 py-4 border-b border-slate-200 flex items-center justify-between">
        <div className="flex space-x-4">
          ...
          >
            <Lock className="w-3 h-3 mr-1" /> Sensitive Data
          </button>
          <button 
      ...
      <div className="flex-1 overflow-auto p-6 bg-slate-50/50">
        ...
        {activeTab === 'sensitive' && (
          <div className="flex flex-col items-center justify-center h-full text-center max-w-md mx-auto">
            ...
            <h3 className="text-lg font-semibold text-slate-800 mb-2">Restricted Access</h3>
            <p className="text-sm text-slate-500 mb-6">
              This area contains sensitive employee data (ID cards, bank accounts). Access is strictly logged and limited to authorized HR personnel.
            </p>
            {['Super Admin', 'HR'].includes(userRole) ? (
        ...
        {activeTab === 'handover' && (
           <div className="bg-white border border-slate-200 rounded-lg p-8 text-center text-slate-500">
             ...
             <p>No active handover checklists.</p>
           </div>
        )}
      </div>
      ...
      <Modal isOpen={isUploadModalOpen} onClose={() => setIsUploadModalOpen(false)} title="Upload Document">
        <form className="space-y-4">
          ...
          <div>
            ...
            <input type="text" placeholder="Select Employee..." className="w-full px-3 py-2 border border-slate-200 rounded-md text-sm focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none" />
          </div>
